const { loadAccount } = require('./utils/common.js');
const SLEEP_INTERVAL = process.env.SLEEP_INTERVAL || 2000;
const PRIVATE_KEY_FILE_NAME =
  process.env.PRIVATE_KEY_FILE || './caller/caller_private_key';
const { abi, networks } = require('./caller/build/contracts/CallerContract.json');
const EthPriceOracle = require('./oracle/build/contracts/EthPriceOracle.json');
...
(async () => {
    ...
    const networkId = await web3js.eth.net.getId();
    const oracleAddress = EthPriceOracle.networks[networkId].address;
    await callerContract.methods
        .setOracleInstanceAddress(oracleAddress)
        .send({ from: ownerAddress });
    setInterval(async () => {
        await callerContract.methods.updateEthPrice()
            .send({ from: ownerAddress });
    }, SLEEP_INTERVAL);
})();
